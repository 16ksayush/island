...
|---|---|
| Responsive **haunted map** — a painted backdrop overlaid with an inline-SVG corridor of glowing portal/door nav-nodes (reflows to a list on phones) | Responsive **archipelago map** — a painted backdrop overlaid with an inline-SVG ocean of isle nav-nodes (reflows to a list on phones) |
...
> **Status:** ✅ Implemented. The backend (FastAPI + **Cloudinary CDN** image source), frontend (SSR Jinja2 + dual themes + audio engine), and the full test suite (**228 passed**) are complete and security-audited. The landing is now a **responsive hybrid map** (M15): a painted raster backdrop per theme overlaid with a high-fidelity inline-SVG scene whose `viewBox` carries the 19 nav-nodes (portals/doors for Horror, isles for Sea), which **reflows to an accessible stacked list on phones** (CSS-toggled at 768px) — no more pixel-pinned hotspots or pannable mobile map. The per-level pages are full-bleed immersive scenes with a framed photo centerpiece, theme ambient decor, and an atmospheric sealed-level fallback (M14, both themes). Images are hosted on **Cloudinary** and served straight from its **keyless public CDN** — one server-side Admin API list discovers what exists (refreshed every 30 min), and the browser loads each image directly from `res.cloudinary.com` with no proxy and no per-request external fetch (M13, supersedes the M12 Drive build-bake). Deploy config (`render.yaml` / `Procfile`) is ready; live deployment (M8) is the remaining step. See [Project Status](#-project-status). *(Note: the `.mp3` audio files are still placeholders.)*
...
- **Responsive hybrid landing (M15)** — each theme paints a full-bleed raster backdrop (the original maps, **kept and reused** as ambient art) behind an inline-SVG scene whose `viewBox` holds the 19 nav-nodes (portals/doors for Horror, isles for Sea); on phones (≤768px) it CSS-toggles to an accessible stacked `.nav-list` of the same 19 targets. It reflows cleanly across PC, tablet, and phone (portrait + landscape) — no horizontal scroll, no drifting hotspots, no pannable map.
- **Dynamic level discovery** — the landing renders a fixed layout of 19 numbered nav-nodes; discovery of whatever numbered folders exist on Cloudinary (under `all ages/{N}`; 5, 12, 19… all work) drives each node's **availability** styling (sealed/sunken for missing levels), not the layout itself.
...
```
Browser (Horror = backdrop + inline-SVG haunted map of portal nav-nodes · Sea = backdrop + inline-SVG archipelago of isle nav-nodes · reflows to a list on phones)
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/` | Landing page; **both themes** = a painted backdrop + an inline-SVG `.nav-map` (`viewBox="0 0 1600 900"`) of 19 nav-nodes that CSS-reflows to a stacked `.nav-list` at ≤768px — **Horror** = haunted corridor of portals, **Sea** = archipelago of isles; discovery drives sealed/sunken styling for missing levels; theme from cookie |
...
│   ├── img/
│   │   ├── horror/        # landing-map.v2.jpg (Horror landing BACKDROP — reused behind the SVG nav-map, M15)
│   │   ├── light/         # landing-map.v2.webp (Sea landing BACKDROP, M15; landing-map.png = design source)
│   │   └── logo/          # brand asset
│   └── audio/
...
├── templates/
│   ├── index.html         # landing: backdrop + inline-SVG nav-map ⇄ mobile nav-list (Horror ⇄ Sea, M15)
...
- [x] **M15 — Responsive hybrid landing (frontend-only)** — the landing is no longer a fixed-aspect raster `<img>` map with 19 %-positioned `.map-hotspot` anchors (which drifted off-art on other screen ratios and turned into a pannable oversized map on phones). It is now a **HYBRID**: a theme-conditional `<div class="nav-backdrop">` paints the **kept** raster map as a full-bleed `cover` backdrop (with a per-arm `<link rel=preload>`), over which an `.atmosphere`/scrim layer and a high-fidelity hand-authored inline `<svg class="nav-map" viewBox="0 0 1600 900">` carry ornate `<use>`-instanced mystical **portal/door** (Horror) / **isle** (Sea) `.nav-node` `<a href="/level/{id}">` targets in one shared coordinate space (art + nodes scale together, no drift). On phones it **CSS-toggles at 768px** to an accessible stacked `.nav-list` of the same 19 `.nav-list-item` rows (≥44px, `display:none` so exactly one representation is in the a11y tree). Removed the `<img>` map + `.map-hotspot` overlay + `coords`/`sea_coords` template dicts + `?calibrate`; **kept** `sea_names` (aria/label source) and **kept** both raster maps (now backdrops, not deleted). `style.css?v=7 → ?v=8` on both templates. **Frontend-only — `app/main.py`, routes, the `/api/levels/{id}/photos` payload, Cloudinary discovery, the audio engine, captions, and the M14 level pages are untouched** (still `{theme, levels}` only). Full suite **228 passed** (adds `tests/test_m15_landing.py`; landing-asset assertions migrated to the new nav idiom). Change set: `templates/index.html`, `static/style.css`, `templates/level.html` (`?v=8` bump), the docs, and `tests/test_m15_landing.py`.

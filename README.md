...
> **Status:** ✅ Implemented. The backend (FastAPI + **Cloudinary CDN** image source), frontend (SSR Jinja2 + dual themes + audio engine), and the full test suite (**220 passed**) are complete and security-audited. The default landing `/` is now a theme-neutral **chest-chooser gateway** (M16) — pick a chest and a ghost/cake reveal zooms you into a theme; the two illustrated landing maps moved to `/map/horror` and `/map/sea` (M9 Horror, M10 Sea), each with clickable hotspots. The per-level pages are full-bleed immersive scenes with a framed photo centerpiece, theme ambient decor, and an atmospheric sealed-level fallback (M14, both themes). Images are hosted on **Cloudinary** and served straight from its **keyless public CDN** — one server-side Admin API list discovers what exists (refreshed every 30 min), and the browser loads each image directly from `res.cloudinary.com` with no proxy and no per-request external fetch (M13, supersedes the M12 Drive build-bake). Deploy config (`render.yaml` / `Procfile`) is ready; live deployment (M8) is the remaining step. See [Project Status](#-project-status). *(Note: the `.mp3` audio files are still placeholders.)*
...
- **Per-level pages** at `/level/{id}`, each a full-bleed, immersive scene styled as a haunted *room* (Horror) or a sunlit *beach/shore* (Sea) — the photo slideshow sits in a themed CSS/SVG frame as the focal centerpiece, with the same ambient decor the landing maps use (drifting ghosts/twinkles/shooting-stars for Horror; balloons/gulls/clouds for Sea) and an atmospheric sealed/missing-level fallback (M14). Both themes carry their own inline-SVG scene on the level page (`.horror-scene` / `.sea-scene`); all decor is `aria-hidden`/inert and frozen under `prefers-reduced-motion`. Frontend-only, pure CSS + inline SVG (no extra image assets).
...
- **Fast by default** — level images are served **directly from Cloudinary's CDN** (no proxy, no per-request external fetch on the hot path), with Cloudinary's own edge caching and optional `f_auto,q_auto` (automatic format + quality) delivery. Discovery reads an in-memory cache populated by the startup Admin list, so a cold start makes **one** Admin call (and zero per request thereafter). The level page shows a slow, scrollable **slideshow** (3s/slide) and each map page (`/map/horror`, `/map/sea`) **prefetches** all level images in the background, so navigation is near-instant after first load.
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/` | **Chest-chooser gateway (M16)** — a theme-NEUTRAL landing (no `.theme-*` class) over a painted backdrop, with a transparent cursive "choose a chest" caption and three symbol-only chests (moon → Horror, half-sun/moon → random, sun → Sea). Opening a chest plays a ghost/cake zoom reveal, writes the `theme` cookie, then navigates to the matching map. Always the gateway; never a map and never a redirect. |
| GET | `/map/horror` | **Horror landing map (M16)** — full-bleed illustrated haunted map with 19 %-positioned clickable hotspots (`?calibrate` outlines them); theme **forced to horror** by the route; deep-linkable standalone. |
| GET | `/map/sea` | **Sea landing map (M16)** — full-bleed illustrated archipelago map (same hotspot layout); theme **forced to sea** by the route; deep-linkable standalone. The in-page theme toggle stays on both maps (switching maps). |
| GET | `/level/{id}` | Dedicated, theme-styled level page (room / beach); theme from cookie |
...
├── static/
│   ├── style.css          # .theme-horror / .theme-sea + scoped .gateway (neutral)  [?v=9]
│   ├── theme.js           # cookie/session + Archive19Theme.choose() + neutral-gateway/forced-map-toggle wiring
│   ├── gateway.js         # M16 chest-chooser: reveal + cookie-before-navigate + reduced-motion skip
│   ├── img/
│   │   ├── gateway/      # gateway-bg.jpg (M16 painted gateway backdrop, ~369 KB)
│   │   ├── horror/        # landing-map.v2.jpg (Horror landing art)
│   │   ├── light/         # landing-map.v2.webp (Sea landing art; landing-map.png = design source)
│   │   └── logo/          # brand asset
│   └── audio/
│       ├── global/        # horror_ambient.mp3, sea_ambient.mp3, gateway_ambient.mp3 (M16, optional)
│       ├── horror/        # level_0.mp3 … level_18.mp3
│       └── sea/           # level_0.mp3 … level_18.mp3
├── templates/
│   ├── gateway.html       # NEW (M16) — theme-neutral chest-chooser at /
│   ├── index.html         # map (served at /map/horror & /map/sea): Haunted Map ⇄ Archipelago Map
...
Build is **complete** through M7 plus the M9 Horror / M10 Sea landing-map redesigns, the M11 dual-theme captions, the M13 **Cloudinary image source** (which supersedes the M12 Drive build-bake), the M14 **atmospheric per-level pages**, the M15 **level-18 song-synced lyrics**, and the M16 **chest-chooser gateway** (the maps moved to `/map/horror` + `/map/sea`) — full suite **220 passed**; deploy config is ready (M8 = live verification, now just needs `CLOUDINARY_URL`). The `.mp3` audio files are still placeholders. Milestones:
...
- [x] **M14 — Dynamic, atmospheric per-level pages (frontend-only)** — `/level/{id}` is no longer a thin framed image floating in a flat dark/empty backdrop; it is now a full-bleed, immersive scene for **both** themes and **both** states. Horror gets a haunted-room environment (`level-env-wall`/`-floor` bands behind the existing `.horror-scene`); the **Sea level page now has its own inline-SVG `.sea-scene`** (the structural twin of `.horror-scene`, with `<radialGradient id="sea-sun-lvl">` + `.sea-silhouette--*` + `level-env-sky`/`-sea`/`-sand` bands) where it previously rendered nothing. The photo slideshow sits in a themed pure-CSS/SVG frame (`.frame-corner`/`-corner--*`) as the focal centerpiece; both themes reuse the landing's ambient decor (ghosts/twinkles/shooting-stars; balloons/gulls/clouds) inside a new neutral `.level-ambient` wrapper; and the sealed/missing-level fallback (`salvaged fallback content` note, restyled as `.level-note`) is atmospheric rather than a void. All decor is `aria-hidden`/inert/no-`tabindex` and frozen under `prefers-reduced-motion`. **Pure CSS + inline SVG — no new image assets**; `style.css?v=6 → ?v=7` on both templates. **Frontend-only — `app/main.py`, routes, the `/api/levels/{id}/photos` payload, Cloudinary discovery, the audio engine, and both landing maps are untouched** (no new SSR var). Full suite **185 passed** (adds `tests/test_m14_level_atmosphere.py`, 21 tests). Change set: `templates/level.html`, `static/style.css`, `templates/index.html` (`?v=7` bump), the docs, `design/` artifacts, and three test files.
- [x] **M15 — Level 18 song-synced lyrics (frontend-only)** — `/level/18` (and **only** that level) shows the birthday song's lyrics in a `<section class="lyrics-sync">` just below the image viewer, in **both** themes (the two `level_18.mp3` tracks are byte-identical — same song). The active line tracks the level audio's `currentTime` via a `timeupdate` hook on `window.Archive19Audio.level.el`; one current line shows large with the upcoming line faint beneath. Timing is **anchored to the official cue sheet** (total 02:40 / 160s; 0:16 vocal entry) as a `SECTIONS` array of absolute-second windows, lines spread evenly per window. Two **cursive display faces** load (Google Fonts, level-18 only): `Pirata One` (gilded gothic, Horror, amber candle-flicker glow) and `Tangerine` (mythical calligraphy, Sea, azure luminance); both honor `prefers-reduced-motion`. **Frontend-only — backend, routes, payloads, Cloudinary discovery, and the audio engine are untouched**; `style.css?v=7 → ?v=8` on both templates. Full suite **185 passed** (the M14 cache-bust test re-pinned `?v=7 → ?v=8`). Change set: `templates/level.html`, `templates/index.html`, `static/style.css`, `tests/test_m14_level_atmosphere.py`, the docs.
- [x] **M16 — Chest-chooser gateway landing** — `GET /` is now a NEW theme-**neutral** gateway (`templates/gateway.html`, `<html data-theme-neutral>`, no `.theme-*` class) rendered over a painted raster backdrop (`static/img/gateway/gateway-bg.jpg`, ~369 KB) + scrim, with a transparent lowered cursive "choose a chest" caption (`Cinzel Decorative`) and three symbol-only chest `<button>`s: **moon → Horror**, **half-sun/moon → random** (`Math.random()<0.5`, the same coin drives the reveal art and the destination), **sun → Sea**. Activating a chest plays a ghost/cake reveal that zooms to fullscreen, calls `Archive19Theme.choose(theme)` (a new no-reload cookie+session writer in `static/theme.js`) to set the `theme` cookie, then navigates — so the destination SSR-paints the chosen theme flash-free. The two illustrated landing maps **moved off `/`** to `GET /map/horror` and `GET /map/sea`, each re-rendering the existing `index.html` with the theme **forced by the route** (`<html data-force-theme="…">`); `/` never shows a map and never redirects. `static/theme.js` gained the gateway neutral-guard (never reconciles on `/`) and forced-theme/map-toggle behavior (the in-page toggle on a map flips to the *other* `/map/{theme}`); a new `static/gateway.js` holds the chest wiring, the `hasNavigated`-guarded `animationend`+fallback-timer navigation, the reduced-motion instant-skip, and an optional gesture-gated, fail-silent gateway ambient (`static/audio/global/gateway_ambient.mp3`). Logo top-left → `/`. **Reveal art note:** shipped as a budgeted painted raster backdrop (like the M9/M10 maps), not the originally-specified pure inline SVG. **`/level/{id}`, `/api/*`, the payloads, Cloudinary discovery, and the audio engine are unchanged**; `style.css?v=8 → ?v=9` on all three templates. The existing map tests were re-pointed from `/` to `/map/horror`/`/map/sea` (incl. two `test_backend.py` theme-context tests + the version-pin v8→v9) and a NEW `tests/test_gateway.py` (30 tests) added. Full suite **220 passed**. Change set: `app/main.py`, `templates/gateway.html` (new), `templates/index.html`, `static/theme.js`, `static/gateway.js` (new), `static/style.css`, `static/img/gateway/gateway-bg.jpg` (new), `static/audio/global/gateway_ambient.mp3` (new), the test suite, the docs.
